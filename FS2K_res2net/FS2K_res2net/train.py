...
from FS2K_res2net.data_utils import FS2KSet, load_checkpoint, save_checkpoint
from FS2K_res2net.model import res2net50
...
def train(hparams):
    ...
    # model
    if train_model == 'res2net':
        model = res2net50().to(device)
    print(model)
    model = model.cuda()

    optimizer = optim.SGD(filter(lambda p: p.requires_grad, model.parameters()), lr=hparams.lr,
            momentum=hparams.momentum, weight_decay=hparams.weight_decay)
    scheduler = lr_scheduler.MultiStepLR(optimizer,
        milestones=[int(e) for e in hparams.milestones.split(',')], gamma=hparams.gamma)
        ...
        if test_anno:
            ...
            with torch.no_grad():
                ...
                for data in test_loader:
                    ...
                    y_pred_df = pd.concat([y_pred_df,y_pred_df_temp])
